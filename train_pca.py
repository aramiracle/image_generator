...
def calculate_features(train_root_dir, features_dir):
    ...
    if os.listdir(features_dir):
        ...
        X_pca_normalized = torch.load(os.path.join(features_dir, features_file_name[0]))
...
def train_pca_generator(train_root_dir, batch_size, learning_rate, epochs, transformed_features):
...
if __name__ == "__main__":
    ...
    # Calculate features and apply PCA if not saved
    print('Starting calculating features and transform it with PCA if it is not saved.')
    os.makedirs(features_dir, exist_ok=True)
    X_pca_normalized = calculate_features(train_root_dir, features_dir)
    ...
    train_pca_generator(train_root_dir, batch_size, learning_rate, epochs, X_pca_normalized)
